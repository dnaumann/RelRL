...
let build_interactive_html lmod lmeth rmod rmeth =
  ...
  p "        <div class=\"row\"><label for=\"guard\">Guard</label><input id=\"guard\" placeholder=\"optional, e.g. diff\"></div>\n";
  p "        <div class=\"row\">\n";
  ...
  p "        left.textContent = 'path=' + (it.path || '') + ' rule=' + it.rule;\n";
  p "        const b = document.createElement('button'); b.textContent = 'Apply'; b.className = 'primary';\n";
  p "        b.onclick = async () => { qs('path').value = it.path || ''; qs('rule').value = it.rule || ''; qs('guard').value=''; await applyRewrite(); };\n";
