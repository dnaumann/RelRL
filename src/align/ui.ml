...
let build_interactive_html lmod lmeth rmod rmeth =
  ...
  p "      const out = qs('verify-out'); out.textContent = '';\n";
  p "      setStatus('Verifying (3-prover portfolio, ' + t + 's/goal); this can take minutes...', null);\n";
  p "      qs('btn-verify').disabled = true;\n";
  p "      try {\n";
  p "        const r = await fetch('/verify?t=' + enc(t), {method:'POST'});\n";
  p "        const txt = await r.text();\n";
  p "        if (!r.ok) { out.textContent = txt; setStatus('Verify failed (HTTP ' + r.status + ').', false); return; }\n";
  ...
  p "      } catch (e) { setStatus(String(e.message || e), false); }\n";
  p "      finally { qs('btn-verify').disabled = false; }\n";
  p "    };\n";
  p "\n";
  p "    (async () => {\n";
