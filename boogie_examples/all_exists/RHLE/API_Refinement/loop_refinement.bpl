/*
https://github.com/rcdickerson/rhle-benchmarks/blob/main/api-refinement/loop-refinement.imp
...
while (sum <= 100 && sum' <= 100)
   invariant sum == sum'; 
  {
    // left program. Call to randOdd
    havoc r;
    assume 0 <= r && r < 10 && r mod 2 == 1;
    sum := sum + r;

   // inserted by chk
   assert (exists v: int :: 0 <= v && v < 10 && r == v);

   // right program. Call to rand()
   havoc r';
   assume 0 <= r' && r' < 10;
   // filter 
   assume r == r'; 
