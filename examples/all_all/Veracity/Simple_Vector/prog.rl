...
bimodule COMMUTE_SEQ (AB | BA) =
  ...
  = Var i: int | i: int in
    ...
    (skip | k := 0);
    WhileR (k < 1000) do variant { [> 1000 - k >] }
        invariant { [> (0 <= k /\ k <= 1000) |> }
        invariant { <| (forall m:int. 0 <= m /\ m < 1000 ->
                      let v = x[999 - m] in z[m] = v) <] /\ [> (forall m:int. 0 <= m /\ m < k ->
                      let v = x[999 - m] in z[m] = v) |> }        
        invariant { (forall i:int|i:int.  [> (0 <= i /\ i < k) |> -> (Agree i) ->  let zi|zi = z[i]|z[i] in zi =:= zi) }
        invariant { <| forall m:int. 0 <= m /\ m < 1000 ->
                      let xm = x[m] in y[m] = xm * xm <]}
        invariant { Agree {x}`any }
