...
Axiom listNth'def :
  ...
  (0%Z <= n)%Z /\ (n < (list.Length.length xs))%Z ->
...
Axiom is_sorted3'def :
  ...
  is_sorted3 xs <->
  ((listNth 0%Z xs) <= q1_)%Z /\ (q1_ <= (listNth 2%Z xs))%Z.

(* Why3 assumption *)
Definition is_permutation3 (l1:Init.Datatypes.list Numbers.BinNums.Z)
    (l2:Init.Datatypes.list Numbers.BinNums.Z) : Prop :=
  let fst1 := listNth 0%Z l1 in
  let fst2 := listNth 0%Z l2 in
  let snd1 := listNth 1%Z l1 in
  let snd2 := listNth 1%Z l2 in
  let third1 := listNth 2%Z l1 in
  let third2 := listNth 2%Z l2 in
...
(* Why3 assumption *)
Definition equals3 (l1:Init.Datatypes.list Numbers.BinNums.Z)
    (l2:Init.Datatypes.list Numbers.BinNums.Z) : Prop :=
  ((listNth 0%Z l1) = (listNth 0%Z l2)) /\
  ((listNth 1%Z l1) = (listNth 1%Z l2)) /\
  ((listNth 2%Z l1) = (listNth 2%Z l2)).
...
Axiom upd_list'def :
  ...
  (0%Z <= k)%Z /\ (k < (list.Length.length xs))%Z ->
  match xs with
  ...
  | Init.Datatypes.cons h t3 =>
      ...
      (~ (k = 0%Z) ->
       ((upd_list k v xs) =
        (Init.Datatypes.cons h (upd_list (k - 1%Z)%Z v t3))))
  end.

(* Why3 assumption *)
Definition add3_3 (l:Init.Datatypes.list Numbers.BinNums.Z) :
    Init.Datatypes.list Numbers.BinNums.Z :=
  upd_list 2%Z ((listNth 2%Z l) + 3%Z)%Z
  (upd_list 1%Z ((listNth 1%Z l) + 3%Z)%Z
   (upd_list 0%Z ((listNth 0%Z l) + 3%Z)%Z l)).
...
Axiom add3_preserves_equals3 :
  forall (l1:Init.Datatypes.list Numbers.BinNums.Z)
    (l2:Init.Datatypes.list Numbers.BinNums.Z),
  equals3 l1 l2 -> equals3 (add3_3 l1) (add3_3 l2).

(* Why3 assumption *)
Definition equals3_bipred (l_s:state) (r_s:state) (pi:t2)
    (l_l:Init.Datatypes.list Numbers.BinNums.Z)
    (r_l:Init.Datatypes.list Numbers.BinNums.Z) : Prop :=
...
Axiom Requires1 : equals3_bipred l_s r_s pi l_l r_l.

Axiom H1 : ((list.Length.length l_l) = 3%Z).

Axiom H2 : ((list.Length.length r_l) = 3%Z).
...
Axiom Assume : is_permutation3 l_l l_sort_ret.

Axiom Assume1 : is_sorted3 o4.

Axiom Assume2 : is_permutation3 r_l o4.

Axiom Assert :
  exists r_shuffle_ret:Init.Datatypes.list Numbers.BinNums.Z,
  equals3 r_shuffle_ret o4.

Parameter r_shuffle_ret: Init.Datatypes.list Numbers.BinNums.Z.

Axiom Assume3 : equals3 r_shuffle_ret o4.
...
Axiom Ensures1 : (r_l1 = r_shuffle_ret).

Axiom Ensures2 : okRefperm l_s r_s pi.

(* Why3 goal *)
Theorem H3 : equals3 (add3_3 l_l1) (add3_3 r_l1).
Proof.

apply add3_preserves_equals3.
