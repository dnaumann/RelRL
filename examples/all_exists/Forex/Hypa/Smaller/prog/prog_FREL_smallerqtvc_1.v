...
Parameter l_time: Numbers.BinNums.Z.

Parameter r_time: Numbers.BinNums.Z.

Axiom H : (pi_rol = (rol pi)) /\ (pi_lor = (lor pi)).

Axiom Requires : okRefperm l_s r_s pi.

Axiom Requires1 : (l_time = r_time).

Parameter l_k: Numbers.BinNums.Z.

Axiom Ensures : (l_k = l_time).

Parameter r_k: Numbers.BinNums.Z.

Axiom Ensures1 : (r_k = r_time).

Parameter l_result: Numbers.BinNums.Z.

Axiom Ensures2 : (l_result = 0%Z).

Parameter r_result: Numbers.BinNums.Z.

Axiom Ensures3 : (r_result = 0%Z).

Parameter r_k1: Numbers.BinNums.Z.

Parameter l_k1: Numbers.BinNums.Z.

Parameter l_result1: Numbers.BinNums.Z.

Parameter r_result1: Numbers.BinNums.Z.

Axiom LoopInvariant : okRefperm l_s r_s pi.

Axiom LoopInvariant1 : (r_result1 <= l_result1)%Z.

Axiom LoopInvariant2 : (0%Z < l_k1)%Z <-> (0%Z < r_k1)%Z.

Axiom H1 : (0%Z < l_k1)%Z.

Parameter l_k2: Numbers.BinNums.Z.

Axiom Ensures4 : (l_k2 = (l_k1 - 1%Z)%Z).

Parameter r_k2: Numbers.BinNums.Z.

Axiom Ensures5 : (r_k2 = (r_k1 - 1%Z)%Z).

Parameter l_b: Init.Datatypes.bool.

Axiom H2 : (l_b = Init.Datatypes.true).

Parameter l_result2: Numbers.BinNums.Z.

Axiom Ensures6 : (l_result2 = (l_result1 + 1%Z)%Z).

(* Why3 goal *)
Theorem smaller'vc : exists r_b:Init.Datatypes.bool, (l_b = r_b).
Proof.
